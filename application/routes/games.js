...
const { unique, calculateGameExp, protect, calculateNeededExp } = require('../helpers/util');
...
const games = (fastify, opts, done) => {
  ...
  const onGameCreate = async (request, reply) => {
    ...
    const { word } = await Word.findOne({ locale }).skip(Math.floor(Math.random() * wordCount));
  ...
  const onWordGuess = async (request, reply) => {
    ...
    const isCorrect = normalizedGuess === word || (isOneLetter && word.includes(normalizedGuess));
    const isAlreadyHandled = isOneLetter && (game.guessed.includes(guess) || game.mistakes.includes(guess));
    if (isCorrect) {
    ...
    const isWordGuessed = wordUniqueLetters.every((letter) => guessed.includes(letter));
  ...
  const onGameGet = async (request, reply) => {
    ...
    const game = await Game.findById(id).populate('user');
    if (!game) return reply.notFound('Game not found');
    const type = game.isFinished ? (game.attemptsLeft === 0 ? 'lose' : 'win') : 'game';
